...
def plot(obj: str, show: bool = True):
    ...
    plt.subplots(figsize=(3, 3*(3/4)))  # 宽度1.9inch，高度按3:4比例
    ...
    plt.legend()
    # 添加标题和标签
    plt.xlabel('加密开销（CPU周期/bit）')
    plt.ylabel(alg_chn)
    # 显示图形
    if show:
        plt.show()
    else:
        plt.savefig(THESIS_PATH+f'enc_ovhd_vs_{obj.lower()}.pdf',
