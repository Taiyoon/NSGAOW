...
def plot(obj: str, show: bool = True):
    ...
    FIGSIZE = 3
    plt.subplots(figsize=(FIGSIZE, FIGSIZE*(3/4)))  # 宽度1.9inch，高度按3:4比例
    ...
    plt.legend()
    # 添加标题和标签
    plt.xlabel('数据平均大小（Mbit）')
    plt.ylabel(alg_chn)
    # 显示图形
    if show:
        plt.show()
    else:
        plt.savefig(THESIS_PATH+f'data_size_vs_{obj.lower()}.pdf',
